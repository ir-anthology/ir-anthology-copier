...
class PDFcopier:
    ...
    def __init__(self, use_wcsp_by_title, use_wcsp_by_doi, use_extracted_by_title, use_extracted_by_doi, dry_run, overwrite_existing):
        self.use_wcsp_by_title = use_wcsp_by_title
        self.use_wcsp_by_doi = use_wcsp_by_doi
        self.use_extracted_by_title = use_extracted_by_title
        self.use_extracted_by_doi = use_extracted_by_doi
        ...
        self.log_directory = "copy_logs" + sep + self.start
        if not exists(self.log_directory): makedirs(self.log_directory)
        with open(self.log_directory + sep + "settings.txt", "w") as settings_file:
            settings_file.write("use_wcsp_by_title: " + str(use_wcsp_by_title) + "\n")
            settings_file.write("use_wcsp_by_doi: " + str(use_wcsp_by_doi) + "\n")
            settings_file.write("use_extracted_by_title: " + str(use_extracted_by_title) + "\n")
            settings_file.write("use_extracted_by_doi: " + str(use_extracted_by_doi) + "\n")
            settings_file.write("dry_run: " + str(dry_run) + "\n")
            settings_file.write("overwrite_existing: " + str(overwrite_existing) + "\n")
    ...
    def sources(self, title, first_author, doi, venue, year):
        ...
        return {"by_doi": {"acm50years": check_path("../sources/acm50yrs/papers-by-doi/" + doi + ".pdf") if doi else None,
                           ...
                           "papers-by-venue-extracted-by-doi": check_path("../sources/papers-by-venue-extracted-by-doi/" + venue + "/" + str(year) + "/" + doi + ".pdf") if self.use_extracted_by_doi and doi else None,
                           "wlgc": check_path("../sources/wlgc/papers-by-doi/" + doi + ".pdf") if doi else None,
                           "wcsp15 (using doi)": check_path("../sources/" + self.wcsp15_doi_path_mapping[doi]) if self.use_wcsp_by_doi and doi and doi in self.wcsp15_doi_path_mapping else None},
                "by_title": {"papers-by-venue-extracted-by-title": check_path("../sources/papers-by-venue-extracted-by-title/" + venue + "/" + str(year) + "/" + title_as_filename + ".pdf") if self.use_extracted_by_title else None,
                             "wcsp15 (using title)": check_path(pdf_src_path_wcsp_per_title) if self.use_wcsp_by_title and title_lowered in self.wcsp15_title_path_mapping and first_author == wscp_first_author and year == wscp_year else None}
                }
    
    def bibliography(self, entry):
        entry_in_unicode = bibtexparser.customization.convert_to_unicode(entry)
        bibkey = entry_in_unicode["ID"]
        title = entry_in_unicode["title"]
        first_author = self.normalize_name_to_ascii(self.get_first_author(entry_in_unicode)) if "author" in entry_in_unicode else None
        doi = entry_in_unicode["doi"].replace("\\", "") if "doi" in entry_in_unicode else None
    ...
    def run(self, venuetype):
        ...
        added_csv_filename = "copy_pdfs_" + self.start + "_" + venuetype + "_added.csv"
                            ...
                            if pdf_src_paths["by_doi"][source_path]:
                                if self.overwrite_existing or not exists(pdf_dst_path):
                                    if not exists(pdf_dst_path):
                                        with open(self.log_directory + sep + added_csv_filename, "a") as added_file:
                                            added_file.write(bibkey + "\n")
        ...
        with open(self.log_directory + sep + missing_csv_filename, "a") as result_csv_file:
        ...
        with open(self.log_directory + sep + results_csv_filename, "a") as result_csv_file:
            csv_writer = writer(result_csv_file, delimiter=",")
        ...
        # write overview
        with open(self.log_directory + sep + results_txt_filename, "w") as logfile:
    ...
    def generate_pdf_ratio_overview(self, pdf_ratio_per_venue_and_year, venuetype):
        pdf_ratio_filename = self.log_directory + sep + "copy_pdfs_" + self.start + "_" + venuetype + "_pdf_ratio.csv"
    ...
    def write_years_not_found_by_dois(self, years_of_entries_not_found_by_doi, venuetype):
        with open(self.log_directory + sep + "years_of_entries_not_found_by_doi_" + venuetype + ".txt", "w") as file:
    ...
    def plot_years_not_found_by_dois(self, years_of_entries_not_found_by_doi, venuetype):
        ...
        plt.savefig(self.log_directory + sep + "years_of_entries_not_found_by_doi_" + venuetype + ".jpg")


if __name__ == "__main__":
    
    pdf_copier = PDFcopier(use_wcsp_by_title=False,
                           use_wcsp_by_doi=True,
                           use_extracted_by_title=True,
                           use_extracted_by_doi=True,
                           dry_run=True,
